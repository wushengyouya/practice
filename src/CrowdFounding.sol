...
contract CrowdFounding {
    ...
    event Launch(address indexed owner, uint256 startTime, uint256 endTime, uint256 goal);
    ...
    event EndCrowdFounding(uint256 indexed id, address indexed from, address indexed to, uint256 amount);
    ...
    //发起众筹
    function launch(uint256 amount, uint256 endTime) public returns (uint256 currentId) {
        //金额必须大于0
        require(amount > 0, "goal amount < 0");
        //结束时间必须大于开始时间
        require(endTime > block.timestamp, "endTime < startTime");
        require(msg.sender != address(0), "current address is the zero address");
        Transaction memory transaction = Transaction(countId, amount, block.timestamp, endTime, 0, false);
        currentId = countId;
    ...
    function crowdFounding(uint256 id, uint256 amount) public {
        ...
        //众筹金额必须大于等于账户余额
        require(token.balanceOf(msg.sender) >= amount, "crowdfounding amount > balanceOf amount");
    ...
    function getCrowdFounding(uint256 id) public view returns (uint256 crowdId, uint256 raiseAmount, address owner) {
    ...
    function endCrowdFounding(uint256 id) public {
        ...
        require(block.timestamp > transaction.endTime, "current time < end time");
        require(msg.sender == ownerCrowds[id], "not owner");
        console.log(transaction.raiseAmount, transaction.goal, transaction.end);
        require(transaction.raiseAmount >= transaction.goal, "not enough goal amount");
